...
/**
 * Performance test for the Emergencia entity.
 */
class EmergenciaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Emergencia entity")
        ...
        .repeat(2) {
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_emergencia_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created emergencia")
                .get("/cadgateway${new_emergencia_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created emergencia")
            .delete("/cadgateway${new_emergencia_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
