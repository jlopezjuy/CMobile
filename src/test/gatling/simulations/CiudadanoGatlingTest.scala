...
/**
 * Performance test for the Ciudadano entity.
 */
class CiudadanoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Ciudadano entity")
        ...
        .repeat(2) {
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ciudadano_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ciudadano")
                .get("/cadgateway${new_ciudadano_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ciudadano")
            .delete("/cadgateway${new_ciudadano_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
